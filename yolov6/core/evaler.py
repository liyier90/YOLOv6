...
class Evaler:
    def __init__(
        ...
        self.ids: List[int]
        self.is_coco: bool
        self.speed_result: torch.Tensor
        self.stride: int
        self.data = data
    ...
    def convert_to_coco_format(self, outputs, imgs, paths, shapes, ids):
        ...
        for i, pred in enumerate(outputs):
            ...
            scale_coords(imgs[i].shape[1:], pred[:, :4], shape, shapes[i][1])
    ...
    def eval_model(self, pred_results, model, dataloader, task):
        """Evaluate models
        For task speed, this function only evaluates the speed of model and
        outputs inference time.
        For task val, this function evaluates the speed and mAP by pycocotools,
        and returns inference time and mAP value.
        """
        LOGGER.info(f"\nEvaluating speed.")
        ...
        if task != "speed" and len(pred_results):
            ...
            avg_map, map50 = cocoEval.stats[:2]
            # Return results
            model.float()  # for training
            if task != "train":
                LOGGER.info(f"Results saved to {self.save_dir}")
            return (map50, avg_map)
        return (0.0, 0.0)
    ...
    def eval_speed(self, task):
        ...
        if task != "train":
            ...
            for n, v in zip(
                ...
                LOGGER.info(f"Average {n} time: {v:.2f} ms")
    ...
    @staticmethod
    def reload_dataset(data) -> Dict[str, Any]:
    ...
    @staticmethod
    def reload_device(device, model, task) -> torch.device:
